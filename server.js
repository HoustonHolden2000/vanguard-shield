...
function calculateRiskScore(scanData) {
  ...
  // 2. Watchlist match (+50)
  if (scanData.dl_number) {
    const watchHit = db.prepare(
      "SELECT reason, severity FROM watchlist WHERE dl_number = ? AND active = 1"
    ).get(scanData.dl_number);
    if (watchHit) {
      score += watchHit.severity === 'RED' ? 50 : 30;
      flags.push(`WATCHLIST_HIT: ${watchHit.reason}`);
    }
  }

  // 3. Cross-site frequency (+20)
  if (scanData.dl_number) {
    const multiSite = db.prepare(`
      SELECT COUNT(DISTINCT site_id) as cnt 
      FROM scans WHERE dl_number = ? AND scan_timestamp > datetime('now', '-7 days')
    `).get(scanData.dl_number);
    if (multiSite && multiSite.cnt >= 3) {
      score += 20;
      flags.push(`MULTI_SITE: ${multiSite.cnt} sites in 7 days`);
    }
  }

  // 4. High frequency same site (+15)
  if (scanData.dl_number && scanData.site_id) {
    const freq = db.prepare(`
      SELECT COUNT(*) as cnt 
      FROM scans WHERE dl_number = ? AND site_id = ? AND scan_timestamp > datetime('now', '-1 day')
    `).get(scanData.dl_number, scanData.site_id);
    if (freq && freq.cnt >= 3) {
      score += 15;
      flags.push(`HIGH_FREQUENCY: ${freq.cnt}x in 24h at this site`);
    }
  }

  // 5. Out-of-state first scan (+10)
  if (scanData.dl_state && scanData.dl_state !== 'TN') {
    const prior = db.prepare("SELECT COUNT(*) as cnt FROM scans WHERE dl_number = ?")
      .get(scanData.dl_number);
    if (!prior || prior.cnt === 0) {
      score += 10;
      flags.push(`FIRST_SCAN_OUT_OF_STATE: ${scanData.dl_state}`);
    }
  }
  ...
  if (hour >= 22 || hour < 5) {
    score += 5;
    flags.push('OFF_HOURS_SCAN');
  }

  // 7. BOL with no carrier ID (+10)
  if (scanData.bol_number && !scanData.carrier_mc_number && !scanData.carrier_dot_number) {
    score += 10;
    flags.push('BOL_NO_CARRIER_ID');
  }

  // 8. No DL number extracted (+25)
  if (!scanData.dl_number) {
    score += 25;
    flags.push('NO_DL_NUMBER');
  }
...
app.post('/api/auth/verify', authenticateToken, (req, res) => {
  ...
  res.json({ 
    user: { ...req.user, site_name: site ? site.name : 'Unassigned' } 
  });
});
...
app.post('/api/scans', authenticateToken, scanUpload, async (req, res) => {
  try {
    ...
    res.json({
      ...
      alert_level: riskResult.alertLevel,
...
app.get('/api/scans', authenticateToken, (req, res) => {
  ...
  // Guards see only their site; admins/supervisors see all
  if (req.user.role === 'guard' && req.user.site_id) {
    where.push('s.site_id = ?');
    params.push(req.user.site_id);
  }
  if (alert_level) {
    where.push('s.alert_level = ?');
    params.push(alert_level);
  }
  if (date_from) {
    where.push('s.scan_timestamp >= ?');
  ...
  const scans = db.prepare(`
    SELECT s.*, u.full_name as guard_name, si.name as site_name
    FROM scans s
...
app.get('/api/watchlist', authenticateToken, (req, res) => {
  const entries = db.prepare(
    "SELECT w.*, u.full_name as added_by_name FROM watchlist w LEFT JOIN users u ON w.added_by = u.id WHERE w.active = 1 ORDER BY w.added_at DESC"
  ).all();
  res.json(entries);
});
...
app.post('/api/watchlist', authenticateToken, (req, res) => {
  ...
  const id = uuidv4();
  db.prepare(
    "INSERT INTO watchlist (id, dl_number, dl_state, first_name, last_name, reason, severity, added_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
  ).run(id, dl_number, dl_state || null, first_name || null, last_name || null, reason, severity || 'YELLOW', req.user.id);
...
app.get('/api/dashboard/stats', authenticateToken, requireRole('admin', 'supervisor'), (req, res) => {
  ...
  const byAlert = db.prepare(`
    SELECT alert_level, COUNT(*) as cnt 
    FROM scans WHERE scan_timestamp > datetime('now', '-7 days')
    GROUP BY alert_level
  `).all();

  const topDrivers = db.prepare(`
    SELECT dl_number, first_name, last_name, COUNT(*) as scan_count, MAX(alert_level) as max_alert
    FROM scans WHERE scan_timestamp > datetime('now', '-30 days') AND dl_number IS NOT NULL
    GROUP BY dl_number ORDER BY scan_count DESC LIMIT 10
  `).all();

  const bySite = db.prepare(`
    SELECT si.name as site_name, COUNT(*) as cnt 
    FROM scans s JOIN sites si ON s.site_id = si.id
    WHERE s.scan_timestamp > datetime('now', '-7 days')
    GROUP BY s.site_id ORDER BY cnt DESC
  `).all();

  const byHour = db.prepare(`
    SELECT CAST(strftime('%H', scan_timestamp) AS INTEGER) as hour, COUNT(*) as cnt
    FROM scans WHERE date(scan_timestamp) = date('now')
    GROUP BY hour ORDER BY hour
  `).all();

  const recentRed = db.prepare(`
    SELECT s.*, u.full_name as guard_name, si.name as site_name
    FROM scans s
    LEFT JOIN users u ON s.guard_id = u.id
    LEFT JOIN sites si ON s.site_id = si.id
    WHERE s.alert_level = 'RED'
    ORDER BY s.scan_timestamp DESC LIMIT 5
  `).all();
  ...
  res.json({
    ...
    byHour,
    recentRed,
    watchlistCount: watchlistCount.cnt
  });
});
...
app.get('/api/users', authenticateToken, requireRole('admin'), (req, res) => {
  const users = db.prepare(`
    SELECT u.id, u.username, u.role, u.full_name, u.site_id, u.created_at, u.last_login, s.name as site_name
...
app.post('/api/users', authenticateToken, requireRole('admin'), (req, res) => {
  ...
  db.prepare("INSERT INTO users (id, username, password_hash, role, full_name, site_id) VALUES (?, ?, ?, ?, ?, ?)")
    .run(id, username, hash, role || 'guard', full_name, site_id || null);
...
app.post('/api/sites', authenticateToken, requireRole('admin'), (req, res) => {
  ...
  const id = uuidv4();
  db.prepare("INSERT INTO sites (id, name, address, city, state, client_name) VALUES (?, ?, ?, ?, ?, ?)")
    .run(id, name, address || null, city || null, state || 'TN', client_name || null);
...
app.get('/api/export/scans', authenticateToken, requireRole('admin', 'supervisor'), (req, res) => {
  ...
  const rows = scans.map(s => [
    s.scan_timestamp, s.guard_name, s.site_name, s.dl_number, s.dl_state,
    `${s.first_name || ''} ${s.last_name || ''}`.trim(), s.date_of_birth, s.expiration_date,
    s.carrier_name, s.carrier_mc_number, s.carrier_dot_number, s.truck_plate,
    s.trailer_number, s.bol_number, s.risk_score, s.alert_level, s.latitude, s.longitude
...
app.listen(PORT, () => {
  console.log(`\n  ╔════════════════════════════════════════╗`);
  console.log(`  ║   VANGUARD SHIELD v1.0                 ║`);
  console.log(`  ║   Running on port ${PORT}                ║`);
  console.log(`  ║   Database: ${DB_PATH.padEnd(24)}   ║`);
  console.log(`  ╚════════════════════════════════════════╝\n`);
});
